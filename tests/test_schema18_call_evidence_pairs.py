...
from scripts.build_schema18_call_evidence_pairs import build
...
def test_evidence_pair_generator_is_deterministic_balanced_and_diverse() -> None:
    rows = generate()
    ...
    assert len(rows) == family_count * 2
    assert Counter(str(row["label"]) for row in rows) == {
        "SAFE": family_count,
        "SCAM": family_count,
    }
    assert {str(row["context_frame"]) for row in rows} == set(CONTEXT_FRAMES)
    assert {str(row["risk_mechanism"]) for row in rows} == set(RISK_MECHANISMS)
    ...
    for pair in grouped.values():
        ...
        safe_ending = str(safe["text"]).rsplit("\n", 1)[1].casefold()
        assert all(cue not in safe_ending for cue in FORBIDDEN_SAFE_ENDING_CUES)
        assert extract_signal_matches(str(scam["text"]))
        assert safe["external_benchmark_text_copied"] is False


def test_schema18_holds_out_scenarios_and_preserves_parent_bytes(tmp_path: Path) -> None:
    ...
    assert manifest["schema18_increment"]["train_rows"] == expected_train
    assert manifest["schema18_increment"]["validation_rows"] == expected_validation
