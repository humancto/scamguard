#!/usr/bin/env python3
"""Build schema v18 from schema v14 plus diverse evidence-action call pairs."""
...
def validate_pair_rows(rows: list[dict[str, object]]) -> None:
    ...
    for row in rows:
        if (
            ...
            raise ValueError(f"unexpected call evidence-pair contract: {row.get('id')}")
        if row.get("label") == "SCAM" and not extract_signal_matches(str(row["text"])):
            raise ValueError(f"call evidence-pair SCAM lacks a signal: {row.get('id')}")
        grouped[str(row.get("pair_id", ""))].append(row)
    if "" in grouped:
        raise ValueError("call evidence-pair data contains an empty pair ID")
    for pair_id, pair in grouped.items():
        if len(pair) != 2 or {str(row["label"]) for row in pair} != {"SAFE", "SCAM"}:
            raise ValueError(f"invalid call evidence pair: {pair_id}")
        if len({str(row["shared_context_sha256"]) for row in pair}) != 1:
            raise ValueError(f"call evidence-pair context hash differs: {pair_id}")
...
def build(
    ...
    if output.exists():
        raise FileExistsError(f"refusing to overwrite schema-v18 output: {output}")
    parent_manifest_path = parent / "manifest.json"
    parent_manifest = json.loads(parent_manifest_path.read_text(encoding="utf-8"))
    if parent_manifest.get("schema_version") != 14:
        raise ValueError("schema-v18 parent must be schema version 14")
    source_manifest = json.loads(source_manifest_path.read_text(encoding="utf-8"))
    if source_manifest.get("source") != SOURCE:
        raise ValueError("call evidence-pair manifest has an unexpected source")
    if file_sha256(source_data) != source_manifest.get("sha256"):
        raise ValueError("call evidence-pair artifact differs from its generator manifest")
    ...
    if not expected_holdouts < actual_scenarios:
        raise ValueError("call evidence-pair holdout scenarios are incomplete")
    ...
    train_pairs = {str(row["pair_id"]) for row in train_increment}
    validation_pairs = {str(row["pair_id"]) for row in validation_rows}
    if train_pairs & validation_pairs:
        raise ValueError("call evidence pair crosses train and validation")
    for partition in (train_increment, validation_rows):
        counts = Counter(str(row["label"]) for row in partition)
        if counts["SAFE"] != counts["SCAM"]:
            raise ValueError(f"call evidence-pair partition is imbalanced: {counts}")
    ...
    if len(source_ids) != len(source_rows) or parent_ids & source_ids:
        raise ValueError("call evidence-pair increment has duplicate or colliding IDs")
    ...
    manifest.update(
        {
            ...
            "schema18_increment": {
                ...
                "train_pair_families": len(train_pairs),
                "validation_pair_families": len(validation_pairs),
                "train_labels": dict(
                ...
                "context_frames": sorted(CONTEXT_FRAMES),
                "risk_mechanisms": sorted(RISK_MECHANISMS),
                "license": "Apache-2.0",
                "provenance": (
                    "original deterministic framing-versus-action counterfactual pairs"
                ),
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/experiments/schema18-call-evidence-pairs/processed"),
    )
