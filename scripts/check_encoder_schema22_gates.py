#!/usr/bin/env python3
"""Apply the frozen schema-22 quality gates without opening sealed artifacts."""
...
def evaluate_gates(
    ...
    for prefix, report_key, threshold_prefix in (
        ("original state", "call_state_validation", "state"),
        ("held-domain MultiDoGO state", "multidogo_state_validation", "multidogo_state"),
    ):
        minimum(
            ...
            f"{threshold_prefix}_harmful_scam_recall_min",
        )
        maximum(
            ...
            f"{threshold_prefix}_routine_safe_fpr_max",
        )
        maximum(
            ...
            f"{threshold_prefix}_verified_safe_fpr_max",
        )
        maximum(
            ...
            f"{threshold_prefix}_unresolved_scam_rate_max",
        )
        minimum(
            ...
            nested(
                ...
                report_key,
                "state_verdict_metrics",
                "ordered_contrast_rate",
            ),
            f"{threshold_prefix}_ordered_contrast_rate_min",
        )
        action_prefix = (
            "action_target"
            if report_key == "call_state_validation"
            else "multidogo_action_target"
        )
        minimum(
        ...
        minimum(
            f"{prefix} action exact match",
            nested(report, report_key, "action_target_metrics", "exact_match_at_0_5"),
    ...
    if not isinstance(domains, dict) or not domains:
        raise ValueError("missing MultiDoGO per-domain metrics")
    ...
    minimum(
        ...
        "bothbosu_latest_window_recall_min",
    )
    maximum(
        ...
        "bothbosu_latest_window_fpr_max",
    )
