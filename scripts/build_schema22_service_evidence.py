#!/usr/bin/env python3
"""Build schema v22 from the safe schema-20 parent and MultiDoGO evidence rows."""
...
def validate_rows(
    ...
    for row in real_train + real_validation:
        if (
            ...
            raise ValueError(f"invalid MultiDoGO real row: {row.get('id')}")
    for row in state_train + state_validation:
        if (
            ...
            raise ValueError(f"invalid MultiDoGO state row: {row.get('id')}")
    if {str(row["source_domain"]) for row in state_train} - set(TRAIN_STATE_DOMAINS):
        raise ValueError("MultiDoGO train state row uses a held-out domain")
    if {str(row["source_domain"]) for row in state_validation} != set(
        VALIDATION_STATE_DOMAINS
    ):
        raise ValueError("MultiDoGO validation state domains differ from the frozen holdout")
    train_families = {str(row["family_id"]) for row in real_train + state_train}
    validation_families = {
        str(row["family_id"]) for row in real_validation + state_validation
    }
    if train_families & validation_families:
        raise ValueError("MultiDoGO family crosses train and validation")
    for rows, expected_split in (
        ...
        if any(row.get("split") != expected_split for row in rows):
            raise ValueError(f"MultiDoGO row has wrong {expected_split} split")


def build(parent: Path, multidogo: Path, output: Path) -> dict[str, object]:
    if output.exists():
        raise FileExistsError(f"refusing to overwrite schema-v22 output: {output}")
    parent_manifest_path = parent / "manifest.json"
    parent_manifest = json.loads(parent_manifest_path.read_text(encoding="utf-8"))
    if parent_manifest.get("schema_version") != 20:
        raise ValueError("schema-v22 parent must be schema version 20")
    source_manifest_path = multidogo / "manifest.json"
    source_manifest = json.loads(source_manifest_path.read_text(encoding="utf-8"))
    if source_manifest.get("source") != SOURCE:
        raise ValueError("MultiDoGO source differs from schema-v22 contract")

    real_train = artifact_rows(
        source_manifest, "real_train", multidogo / "multidogo_real_train.jsonl"
    )
    state_train = artifact_rows(
        source_manifest, "state_train", multidogo / "multidogo_state_train.jsonl"
    )
    ...
    if len(all_ids) != len(set(all_ids)):
        raise ValueError("schema-v22 increment has a duplicate or parent-colliding ID")

    output.mkdir(parents=True)
    combined_train = parent_train + increment
    write_jsonl(output / "train.jsonl", combined_train)
    write_jsonl(output / "multidogo_call_validation.jsonl", real_validation)
    ...
    counts.update(
        {
            ...
            "multidogo_call_validation": len(real_validation),
    ...
    manifest.update(
        {
            ...
            "schema22_increment": {
                ...
                "human_dialogue_train_rows": len(real_train),
                "human_dialogue_validation_rows": len(real_validation),
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/experiments/schema22-service-evidence/processed"),
    )
    args = parser.parse_args()
    print(json.dumps(build(args.parent, args.multidogo, args.output), indent=2))
