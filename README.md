...
| ModernBERT-base, 149M (schema v25 full calls + phone) | rejected rights-clear data expansion | 602.1 MB training artifact | 20/38 gates; 15.39 ms end-to-end MPS p95, but MultiDoGO SAFE FPR 16.96%, BothBosu 85.82% recall / 9.15% FPR, and phone-source SAFE FPR 77.65% |
...
| Qwen3.5-0.8B, stage-7 phone generalization | frozen training experiment; not a result yet | Stage-3 adapter initialization | 26,150 training rows, zero over 640 tokens, zero held overlap, `1e-6` learning rate; validation/test remain outside fitting and publication is forbidden |
