...
| Sub-1B serious specialist | **Partial** | Frozen Qwen3.5-0.8B base revision; rejected 35/39 stage-2, 36/39 stage-3, and 33/39 stage-4 receipts; tested dev-only stage-3/stage-4 interpolation in [`scripts/analyze_qwen_score_blend.py`](../scripts/analyze_qwen_score_blend.py) | Stage 4 improved prior-open BothBosu recall to 82.98% but broadly regressed false positives; leakage-safe interpolation selected stage 3 unchanged. Build a single-adapter stage 5 that preserves stage-3 precision while recovering dialogue recall, then rerun unchanged BF16 gates before any quantization or sealed-primary evaluation. |
