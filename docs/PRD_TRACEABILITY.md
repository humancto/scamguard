...
| Sub-1B serious specialist | **Partial** | Frozen Qwen3.5-0.8B base revision; rejected 35/39 stage-2, 36/39 stage-3, and 33/39 stage-4 receipts; score and weight interpolation rejected with dev-only selection; precision-weighted stage-5 config frozen | Train Stage 5 from Stage 3, then rerun the unchanged 39 BF16 gates. Quantization and sealed-primary evaluation remain forbidden unless one artifact passes every regression gate. |
